...
4. Identify the submission files:
   ...
   - model-specific tests, if present; and
   - every file under `reports/<model>/`.
   Include model-only dependencies when required for the submitted model to
   run. Exclude local summary configuration, other models, unrelated working
   tree changes, and generated files outside the selected report.
...
7. Stage only the submission files and only model-related hunks in shared
   registry files. Reports are ignored by Git, so force-add
   `reports/<model>/`. Review the staged file list and diff before committing.
   Unstage anything unrelated.
8. Verify that the selected model still resolves from `models.MODEL_REGISTRY`
   and that the report remains complete. Do not create an empty commit. Commit
   with the message `Submit <model> model and report`.
